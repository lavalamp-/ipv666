...
func PingScan(conf *config.Configuration, inputFile string, outputFile string, bandwidth string) (string, error) {

	log.Printf("Performing ping scan on addresses defined in %s\n", inputFile)
  ...
  for finished == false {
    ...
    select {
      ...
      case ip := <-ips:
        ...
        if err != nil {
          log.Fatal("error encoding ICMP echo packet with destination %s (%s)", ip, err)
          return "", err
        }
        seq += 1
        ...
        if t != lastStatus {
          lastStatus = t
          log.Printf("Ping-scanned %d addresses (%d hits, %d packets/second)\n", count, atomic.LoadUint64(&hitCount), lastSecondCount)
          lastSecondCount = 0
        }
...
func PingScanFromConfig(conf *config.Configuration, inputFile string, outputFile string) (string, error) {
	return PingScan(conf, inputFile, outputFile, conf.PingScanBandwidth)
}
